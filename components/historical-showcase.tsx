...
// Maps site index → [structureImg, girihImg]
const SITE_IMAGES = [
  ["/worldexamples/topkapi-structure.jpg",          "/worldexamples/topkapi-girih.jpeg"],
  ["/worldexamples/bursa-greenmosque-structure.png", "/worldexamples/bursa-greenmosque-girih.jpeg"],
  ["/worldexamples/gunbadi-kabud-structure.png",     "/worldexamples/gunbadi-kabud-girih.jpeg"],
  ["/worldexamples/darbi-imam-structure.jpg",        "/worldexamples/darbi-imam-girih.png"],
  ["/worldexamples/itimadud-daulah-structure.jpg",   "/worldexamples/itimadud-daulah-girih.png"],
]
    ...
    <svg viewBox="0 0 200 200" className={`absolute opacity-[0.03] ${className}`} fill="none" stroke="currentColor">
      <g strokeWidth="0.5">
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          return (
            <g key={i}>
              <line x1={cx+r1*Math.cos(a1)} y1={cy+r1*Math.sin(a1)} x2={cx+r2*Math.cos((a1+a2)/2)} y2={cy+r2*Math.sin((a1+a2)/2)} />
              <line x1={cx+r2*Math.cos((a1+a2)/2)} y1={cy+r2*Math.sin((a1+a2)/2)} x2={cx+r1*Math.cos(a2)} y2={cy+r1*Math.sin(a2)} />
            </g>
          )
        })}
...
type Site = typeof import("@/lib/i18n").translations["tr"]["sites"][0]

function ShowcaseSection({ site, index }: { site: Site; index: number }) {
  const sectionRef = useRef<HTMLDivElement>(null)
  const isInView = useInView(sectionRef, { once: true, margin: "-100px" })
  const { scrollYProgress } = useScroll({ target: sectionRef, offset: ["start end", "end start"] })
  const parallaxY = useTransform(scrollYProgress, [0, 1], [50, -50])
  const isEven = index % 2 === 0
  const [structureImg, girihImg] = SITE_IMAGES[index]
        ...
        <div className={`flex flex-col gap-8 lg:flex-row lg:items-center lg:gap-16 ${isEven ? "" : "lg:flex-row-reverse"}`}>
          ...
          <motion.div className="relative flex-1"
            ...
            <motion.div className="relative aspect-[4/3] overflow-hidden rounded-lg border border-primary/20 bg-secondary/30" style={{ y: parallaxY }}>
              <Image src={structureImg} alt={site.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
              {/* Subtle overlay */}
          ...
          <div className="flex flex-1 flex-col gap-8">
            ...
            <motion.div className="relative"
              ...
              <motion.div className="relative aspect-square max-w-sm overflow-hidden rounded-lg border border-primary/30 bg-secondary/20"
                whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
                <Image src={girihImg} alt={`${site.name} girih pattern`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 384px" />
    ...
    <div ref={containerRef} id="about" className="relative overflow-hidden bg-background">
      ...
      <div className="relative py-24 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="mx-auto max-w-3xl px-4">
          <span className="mb-4 inline-block font-[Inter] text-xs font-medium uppercase tracking-widest text-primary">{t.historicalBadge}</span>
          <h2 className="mb-6 font-[Playfair_Display] text-4xl font-bold text-foreground md:text-5xl lg:text-6xl">
            ...
            <span className="bg-gradient-to-r from-primary via-gold-light to-primary bg-clip-text text-transparent">{t.historicalTitle2}</span>
          </h2>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative py-24 text-center">
        <div className="mx-auto max-w-2xl px-4">
          ...
          <motion.a href="/generate" className="inline-flex items-center gap-2 rounded-full bg-primary px-8 py-4 font-[Inter] font-semibold text-primary-foreground transition-all hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/25"
